...
export async function POST(
  ...
  try {
    ...
    // Add this post to user's accepted works
    if (!user.acceptedWorks) {
      user.acceptedWorks = [];
    }

    // Check if work already exists in user's accepted works
    const workExists = user.acceptedWorks.some(
      (work: { post: string; acceptedAt: Date; status: string }) =>
        work.post.toString() === postId
    );

    if (!workExists) {
      user.acceptedWorks.push({
        post: postId,
        acceptedAt: new Date(),
        status: "processing",
      });
    ...
    // Log the acceptance (optional)
    console.log(`User ${user._id} accepted work ${postId}`);
