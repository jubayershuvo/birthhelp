...
type RegexSearchQuery = Filter<Document> & {
  ...
  try {
    ...
    if (!q && Object.keys(baseFilter).length === 0 && exprAnd.length === 0) {
      return NextResponse.json(
        {
          error:
            "Provide at least one indexed filter or year range if q is not used",
    ...
    if (!hint) {
      ...
      const hasGender =
        typeof baseFilterMap.gender === "string" &&
        (baseFilterMap.gender as string).trim() !== "";
      const hasPersonNameEn =
        typeof baseFilterMap.personNameEn === "string" &&
        (baseFilterMap.personNameEn as string).trim() !== "";
      const hasPersonNameBn =
        typeof baseFilterMap.personNameBn === "string" &&
        (baseFilterMap.personNameBn as string).trim() !== "";
    ...
    const sentData = encryptFile(response);
    const arrayBuffer = new Uint8Array(sentData).buffer;
