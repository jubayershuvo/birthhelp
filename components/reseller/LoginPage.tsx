...
function LoginPage() {
  ...
  const {reseller, isResellerLoggedIn} = useAppSelector((state) => state.resellerAuth);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await axios.post(`/api/reseller/login`, {
        email,
        password,
      });


      dispatch(resellerLogin(res.data.reseller));
  ...
  useEffect(() => {
    if (isResellerLoggedIn) {
      ...
      toast.success(`Welcome back, ${reseller.name}!`);
    }
  }, [isResellerLoggedIn]);
